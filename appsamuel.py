...
def training():
    if request.method== "POST":
        ...
        token = ' '.join(map(str, word_seq_train))

        # contoh hasil preprocessing-tokenizing
        n = 100
        cth = processed_docs_train['Text Tweet'][n]
        ...
        cth_stopwords = processed_docs_train['stopwords'][n]
        ...
        Y_train = processed_docs_train['Sentiment']
        Y_test = processed_docs_val['Sentiment']

        #Pembuatan model identifikasi aspk [KHUSUS PENGIRIMAN]
...
def testing():
	if request.method=="POST":
		...
		for i in range (len(df_hasil)):
			data_output.append({
					'id': str(i+1),
                    'teks': str(df_hasil['Text Tweet'][i]),
                    'pengiriman': str(df_hasil['Sentiment'][i]),
                    'pengiriman_pred': str(df_hasil['Sentiment_pred'][i]),
                })
