...
testSize = 0.2 #Pembagian ukuran datatesing

MAX_NB_WORDS = 100000 #Maximum jumlah kata pada vocabulary yang akan dibuat
max_seq_len = 11 #Panjang kalimat maximum sesuai datasetku
...
def read_data(filename):
    ...
    label_dict = {'Non-bullying':0, 'Bullying':1, '-':99}
    df['B_Buli'] = df['Kategori'].replace(label_dict)
...
def model_testing(file_test):
    ...
    # Encoding Categorical Data (Mengubah data kategorikal menjadi angka)
    label_dict = {'negative':0, 'positive':1, '-':99}
    df['Kategori'] = df['Kategori'].replace(sentimen_dict)
