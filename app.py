...
def training():
    if request.method== "POST":
        ...
        n = 100
        cth = processed_docs_train['Komentar'][n]
        ...
        Y_train = processed_docs_train['Kategori']
        Y_test = processed_docs_val['Kategori']
...
def testing():
	if request.method=="POST":
		...
		for i in range (len(df_hasil)):
			data_output.append({
					'id': str(i+1),
                    'teks': str(df_hasil['Komentar'][i]),
                    'pengiriman': str(df_hasil['Kategori'][i]),
                    'pengiriman_pred': str(df_hasil['Kategori_pred'][i]),
                })
